...
const parseMessage_1 = require("../utils/parseMessage");
const { events, replicants } = context_1.default;
const { chat: { cheermotes }, } = replicants;
const parseCheermotesFromTwitch = (message) => parseMessage_1.parseCheermotes(message, cheermotes.value);
...
exports.default = (chat) => {
    ...
    chat.on('message', (channel, userstate, messageText) => {
        const message = parseMessage_1.getMessageDetails(messageText, userstate);
        const user = parseMessage_1.getUserDetails(userstate);
        send({
            action: message.type,
            payload: {
                channel,
    ...
    chat.on('cheer', (channel, userstate, messageText) => {
        const user = parseMessage_1.getUserDetails(userstate);
        const message = parseMessage_1.getMessageDetails(messageText, userstate);
        message.tokens = parseMessage_1.parseTokens(message.tokens, (token) => parseCheermotesFromTwitch(token));
        send({
            action: 'cheer',
            payload: {
                channel,
    ...
    chat.on('resub', (channel, username, months, messageText, _, extra = {}) => {
        const message = parseMessage_1.getMessageDetails(messageText);
        send({
            scope: 'channel',
            action: 'subscription',
            payload: {
